...
//Mine
import de.unikoblenz.west.splodge.PredGraph;
...
public class Splodge implements Closeable {
  ...
  //Stores results
  private Set<ArrayList<PredGraph>> predicatesEnc;
  ...
  public long generateQueries(JoinPattern joinPattern, int numberOfJoins, int numberOfDataSources,
    ...
    //CHANGE TO ARRAY[long[]]
//     long[] predicatesEnc = new long[numberOfJoins + 1];
    predicatesEnc = new HashSet<ArrayList<PredGraph>>();
    ...
    if ((numberOfJoins + 1) >= numberOfDataSources) {
      switch (joinPattern) {
        case SUBJECT_OBJECT_JOIN:
            //Add nb joins+ 1 as nbTriples/query
...
//                   maximumNumberOfSelectedTriples, timeout);
          generatePathShapedQuery(/*predicatesEnc,*/
                  numberOfDataSources, requiredNumberOfSelectedTriples,
                  maximumNumberOfSelectedTriples, timeout, numberOfJoins + 1);
          break;
        case SUBJECT_SUBJECT_JOIN:
//           estimatedNumberOfTouchedTriples = generateStarShapedQuery(predicatesEnc,
//                   numberOfDataSources, requiredNumberOfSelectedTriples,
//                   maximumNumberOfSelectedTriples, timeout);
          generateStarShapedQuery(/*predicatesEnc,*/
                  numberOfDataSources, requiredNumberOfSelectedTriples,
                  maximumNumberOfSelectedTriples, timeout, numberOfJoins + 1);
          break;
        default:
...
//     if (estimatedNumberOfTouchedTriples < requiredNumberOfSelectedTriples) {
    ...
    //FOR EACH predicates in ARRAY
    for( ArrayList<PredGraph> apg : predicatesEnc){
        String[] predicates = new String[apg.size()];
        String[] graphs = new String[apg.size()];
        for (int i = 0; i < apg.size(); i++) {
            predicates[i] = dictionary.decode(apg.get(i).getPredicate());
            graphs[i] = dictionary.decode(apg.get(i).getGraph());
            assert predicates[i] != null : "index " + i + " of " + apg;
        }
        queryGenerator.createQuery(queryFileName,
                estimatedNumberOfTouchedTriples / (double) sgpoIndex.getNumberOfTriples(), joinPattern,
                limit, predicates, graphs);
    }
    //END FOR EACH
//     return estimatedNumberOfTouchedTriples;
        //RETURN SIZE OF ARRAY
        return predicatesEnc.size();
  }
  ...
  private String getQueryFileName(JoinPattern joinPattern, int numberOfJoins,
    ...
    //CHANGE long[] TO ARRAY[long[]] + nb Triple/query, changed return to void
  private void generatePathShapedQuery(/*Set<ArrPredGraph> queryResults,*/ int numberOfDataSources,
          long requiredNumberOfSelectedTriples, long maximumNumberOfSelectedTriples, int timeout, int nbTriples) {
          //Change arg
//     long[] graphs = new long[predicates.length];
    long[] graphs = new long[nbTriples];
        // Change arg
//     long[] vertices = new long[predicates.length + 1];
    //New var
    long[] predicates = new long[nbTriples];
    long[] vertices = new long[nbTriples + 1];
    long number = 0;
...
//           return touchedTriples;
            //ADD TO ARRAY reset predicates
            //Add in path
//             if( queryResults.add(new ArrPredGraph(predicates,graphs)) ){
...
//     return -1;
    //RETURN ARRAY OR NO RETURN (NOT THAT USEFUL)
  }
  ...
  private long findPath(int currentIndex, long[] predicates, long[] graphs, long[] vertices,
    ...
    if (currentIndex == predicates.length) {
      if ((requiredNumberOfSelectedTriples <= numberOfTouchedTriplesSoFar)
        ...
        //Add to results
        ArrayList<PredGraph> newElt = new ArrayList<PredGraph>();
        for(int i =0; i<predicates.length; ++i){
            newElt.add(new PredGraph(predicates[i], graphs[i]));
        }
        predicatesEnc.add(newElt);
        ...
        for (long[] snpg : snpgIterable) {
          ...
          for (long[] sgpo : sgpoIndex.getObject(snpg[0], snpg[2], snpg[3])) {
            ...
            long touchedTriples = findPath(currentIndex + 1, predicates, graphs, vertices,
                    ...
                    maximumNumberOfSelectedTriples, abortionTime);
                //Removed to check all possible paths
//             if ((requiredNumberOfSelectedTriples <= touchedTriples)
//                     && (touchedTriples <= maximumNumberOfSelectedTriples)) {
//               return touchedTriples;
//             }
    ...
    //PREDICATES TO ARRAY[long[]] ADD NUMBER OF JOINS, changed return to void
  private void generateStarShapedQuery(/*Set<ArrPredGraph> queryResults,*/ int numberOfDataSources,
          long requiredNumberOfSelectedTriples, long maximumNumberOfSelectedTriples, int timeout, int nbTriples) {
    long number = 0;
    long numberOfSubjects = sortedSubjectSet.size();
    //tmp array predicate long[]
    long[] predicates = new long[nbTriples];
    for (long[] nSubject : new ByteArray2LongArrayIterator(sortedSubjectSet.reverseIterator())) {
      System.out.println("checking " + (++number) + "/" + numberOfSubjects + " subjects");
...
//         return numberOfUsedTriples;
            //ADD TO ARRAY PREDICATES
            //NEW PREDICATES LONG[]
//             queryResults.add(new ArrPredGraph(predicates,graphs));
            predicates = new long[nbTriples];
      }

    }
//     return -1;
    //RETURN ARRAY OR NOTHING
  }
  ...
  private long findStar(int currentIndex, long[][] incidenceList, int firstIncidenceIndex,
    ...
    if (numberOfUsedTriples > maximumNumberOfSelectedTriples) {
      ...
      if ((requiredNumberOfSelectedTriples <= numberOfUsedTriples)
        ...
        //Because {s1 p1 o2,s1 p2 o2}=={s1 p2 o3,s1 p1 o1} with p1 in G1 and p2 in G2
        //Need to sort (graph first) to find duplicate in Set
        newElt.sort(null);
        predicatesEnc.add(newElt);
        return numberOfUsedTriples;
      } else {
